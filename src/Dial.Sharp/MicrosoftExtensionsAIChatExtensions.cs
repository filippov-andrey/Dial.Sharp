...
public static class MicrosoftExtensionsAiChatExtensions
{
    ...
    public static ChatTool AsOpenAiChatTool(this AIFunctionDeclaration function) =>
        DialChatClient.ToOpenAiChatTool(function ?? throw new ArgumentNullException(nameof(function)));

    /// <summary>
    /// Creates an OpenAI <see cref="OpenAI.Chat.ChatResponseFormat"/> from a <see cref="Microsoft.Extensions.AI.ChatResponseFormat"/>.
    /// </summary>
    /// <param name="format">The format.</param>
    /// <param name="options">The options to use when interpreting the format.</param>
    /// <returns>The converted OpenAI <see cref="OpenAI.Chat.ChatResponseFormat"/>.</returns>
    public static OpenAI.Chat.ChatResponseFormat? AsOpenAiChatResponseFormat(
        this ChatResponseFormat? format, ChatOptions? options = null) =>
        DialChatClient.ToOpenAiChatResponseFormat(format, options);
    ...
    public static IEnumerable<OpenAI.Chat.ChatMessage> AsOpenAiChatMessages(
        this IEnumerable<ChatMessage> messages, ChatOptions? options = null) =>
        DialChatClient.ToOpenAiChatMessages(messages ?? throw new ArgumentNullException(nameof(messages)), options);
    ...
    public static ChatCompletion AsOpenAiChatCompletion(this ChatResponse response)
    {
        ...
        var role = ToChatMessageRole(lastMessage?.Role);

        var finishReason = ToChatFinishReason(response.FinishReason);
        ...
        return OpenAIChatModelFactory.ChatCompletion(
            response.ResponseId,
            finishReason,
            new ChatMessageContent(DialChatClient.ToOpenAiChatContent(lastMessage?.Contents ?? [])),
            toolCalls: toolCalls,
            role: role,
            createdAt: response.CreatedAt ?? default,
            model: response.ModelId,
            usage: usage,
            outputAudio: lastMessage?.Contents.OfType<DataContent>().Where(dc => dc.HasTopLevelMediaType("audio"))
                .Select(a => OpenAIChatModelFactory.ChatOutputAudio(new BinaryData(a.Data))).FirstOrDefault(),
        ...
        static IEnumerable<ChatMessageAnnotation> ConvertAnnotations(IEnumerable<AIContent>? contents)
        {
            ...
            foreach (var content in contents)
            {
                ...
                foreach (var annotation in content.Annotations)
                {
                    ...
                    if (citation.AnnotatedRegions?.OfType<TextSpanAnnotatedRegion>().ToArray() is
                        { Length: > 0 } regions)
                    {
                        foreach (var region in regions)
                        {
                            yield return OpenAIChatModelFactory.ChatMessageAnnotation(region.StartIndex ?? 0,
                                region.EndIndex ?? 0, citation.Url, citation.Title);
                        }
                    }
                    else
                    {
                        yield return OpenAIChatModelFactory.ChatMessageAnnotation(webResourceUri: citation.Url,
                            webResourceTitle: citation.Title);
                    }
                }
            }
        }
    ...
    public static async IAsyncEnumerable<StreamingChatCompletionUpdate> AsOpenAiStreamingChatCompletionUpdatesAsync(
        this IAsyncEnumerable<ChatResponseUpdate> responseUpdates,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(responseUpdates);
        ...
        await foreach (var update in responseUpdates.WithCancellation(cancellationToken).ConfigureAwait(false))
        {
            ...
            var toolCallUpdates = update.Contents.OfType<FunctionCallContent>().Select((fcc, index) =>
                    OpenAIChatModelFactory.StreamingChatToolCallUpdate(
                        ...
                        functionArgumentsUpdate: new BinaryData(JsonSerializer.SerializeToUtf8Bytes(fcc.Arguments,
                            AIJsonUtilities.DefaultOptions.GetTypeInfo(typeof(IDictionary<string, object?>))))))
                .ToList();

            yield return OpenAIChatModelFactory.StreamingChatCompletionUpdate(
                update.ResponseId,
                new(DialChatClient.ToOpenAiChatContent(update.Contents)),
    ...
    public static IEnumerable<ChatMessage> AsChatMessages(
        this IEnumerable<OpenAI.Chat.ChatMessage> messages)
    {
        ArgumentNullException.ThrowIfNull(messages);

        foreach (var message in messages)
        {
            ChatMessage resultMessage = new()
            {
            ...
            switch (message)
            {
                case AssistantChatMessage acm:
                    ...
                    DialChatClient.ConvertContentParts(acm.Content, resultMessage.Contents);
                    foreach (var toolCall in acm.ToolCalls)
                    {
                        var fcc = DialClientExtensions.ParseCallContent(toolCall.FunctionArguments, toolCall.Id,
                ...
                case UserChatMessage ucm:
                    ...
                    DialChatClient.ConvertContentParts(ucm.Content, resultMessage.Contents);
                    break;

                case DeveloperChatMessage dcm:
                    resultMessage.Role = ChatRole.System;
                    resultMessage.AuthorName = dcm.ParticipantName;
                    DialChatClient.ConvertContentParts(dcm.Content, resultMessage.Contents);
                    break;

                case SystemChatMessage scm:
                    resultMessage.Role = ChatRole.System;
                    resultMessage.AuthorName = scm.ParticipantName;
                    DialChatClient.ConvertContentParts(scm.Content, resultMessage.Contents);
                    break;
    ...
    public static ChatResponse
        AsChatResponse(this ChatCompletion chatCompletion, ChatCompletionOptions? options = null) =>
        DialChatClient.FromOpenAiChatCompletion(
            chatCompletion ?? throw new ArgumentNullException(nameof(chatCompletion)), options);
    ...
    public static IAsyncEnumerable<ChatResponseUpdate> AsChatResponseUpdatesAsync(
        this IAsyncEnumerable<StreamingChatCompletionUpdate> chatCompletionUpdates,
        ChatCompletionOptions? options = null, CancellationToken cancellationToken = default) =>
        DialChatClient.FromOpenAiStreamingChatCompletionAsync(
            chatCompletionUpdates ?? throw new ArgumentNullException(nameof(chatCompletionUpdates)), options,
            cancellationToken);
    ...
    /// <summary>Converts the <see cref="Microsoft.Extensions.AI.ChatFinishReason"/> to a <see cref="OpenAI.Chat.ChatFinishReason"/>.</summary>
    private static OpenAI.Chat.ChatFinishReason ToChatFinishReason(
        ChatFinishReason? finishReason) =>
        finishReason?.Value switch
        {
            "length" => OpenAI.Chat.ChatFinishReason.Length,
            "content_filter" => OpenAI.Chat.ChatFinishReason.ContentFilter,
            "tool_calls" => OpenAI.Chat.ChatFinishReason.ToolCalls,
            "function_call" => OpenAI.Chat.ChatFinishReason.FunctionCall,
            _ => OpenAI.Chat.ChatFinishReason.Stop,
        };
}